...
module Pod
  ...
  class ProjectManipulator
    ...
    def run
      ...
      add_lib_podspec_metadata
      add_fmk_podspec_metadata
    ...
    def add_lib_podspec_metadata
      project_metadata_item = @project.root_object.main_group.children.select { |group| group.name == "Lib Podspec Metadata"}.first
      project_metadata_item.new_file "../lib/" + @configurator.pod_name  + ".podspec"
    end

    def add_fmk_podspec_metadata
      project_metadata_item = @project.root_object.main_group.children.select { |group| group.name == "FMK Podspec Metadata"}.first
      project_metadata_item.new_file "../fmk/" + @configurator.pod_name  + ".podspec"
    end
